...
// GenerateTools creates TypeScript tool definitions for opencode to load
// This generates a .opencode/tool/iteratr.ts file that exposes all session management tools
func GenerateTools(dataDir, sessionName string, force bool) error {
	toolPath := ".opencode/tool/iteratr.ts"
	...
	// Resolve absolute data directory path for tools to use
	...
	// Tools use the iteratr CLI directly to interact with the embedded NATS store
	content := fmt.Sprintf(`
